...
function AddCategory({ hideCatConfig, handleCloseCat }) {
  const [updatedCat, setUpdatedCat] = useState({});
  const [updatedDuration, setUpdatedDuration] = useState({});
  const [saveMemberDetails, setSaveMemberDetails] = useState({});
  
  const handleInput = (value, type) => {
    if (type === "description" || type === "createdBy" || type === "modifiedBy") {
      console.log("fisr if")
    ...
    } else if (type === "memberName" || type === "emailId" || type === "phoneNo") {
      console.log("tied if")
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogContent>
          ...
          {CommonTypography({ fontWeight: 600, label: "Member Name" })}
          {commonTextField(
            {
              id: "fullWidth",
              className: "BoxShadow",
